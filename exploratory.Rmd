...
Dimensions: [individuals x QTLs], e.g. [1000 x 2500]
...
#geno[2,]
```
Genetic map. Contains ALL 20000 SNPs, including QTL (which causally impact trait) and other markers (unrelated mutations). The value at each index contains the distance in MORGANS from the start of the chromosome.
...
final_map[[2]]
```
Compute a list of all markers (excluding QTL). Total of 16894.
...
train_set <- snp_geno[1:train_end,]
